...
export function ExpensesPage({ expenses, stats, userId }: ExpensesPageProps) {
  ...
  const getCategoryColor = (cat: string) => {
    switch (cat) {
      case "Shop Rent": return "text-blue-700 bg-blue-50 border-blue-200";
      case "Transport Cost": return "text-amber-700 bg-amber-50 border-amber-200";
      case "Staff Salary": return "text-emerald-700 bg-emerald-50 border-emerald-200";
  ...
  const getCategoryIconColor = (cat: string) => {
    switch (cat) {
      case "Shop Rent": return "text-blue-500 bg-blue-50 border-blue-200";
      case "Transport Cost": return "text-amber-500 bg-amber-50 border-amber-200";
      case "Staff Salary": return "text-emerald-500 bg-emerald-50 border-emerald-200";
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <PageHeader
          ...
          description="Log daily running cost, transport, shop rent, staff wages, and double-entry vault allocations."
        />
        <AddExpenseModal userId={userId} />
      </div>
              ...
              {filteredExpenses.length ? (
                filteredExpenses.map((e) => (
                  <tr key={e.id} className="hover:bg-zinc-50/70 transition-colors">
                    ...
                    <td className="px-4 py-3 text-right font-bold text-zinc-900">{formatNPR(Number(e.amount))}</td>
